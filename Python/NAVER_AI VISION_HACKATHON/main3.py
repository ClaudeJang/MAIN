...
def bind_model(model):
    ...
    def infer(queries, db):
        ...
        query_img /= 255 # rescale
        reference_img = reference_img.astype('float32')
        reference_img /= 255 # rescale
        ...
        else: # 70번 부터 81번 추가 OOM 오류 해결위한 코드
            # batch_size = 64 # 기존 128
...
if __name__ == '__main__':
    ...
    model.add(Dropout(0.5))
    model.add(Dense(num_classes))
    model.add(Activation('softmax'))  # softmax는 각 항목에 대한 확률을 계산. 확률을 합하면 1이 됨.
    model.summary()
