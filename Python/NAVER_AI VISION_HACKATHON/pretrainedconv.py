...
if __name__ == '__main__':
    ...
    x1 = Conv2D(64, (3, 3))(input_shape = input_shape)
    ...
    # load vgg16 without dense layer and with a no dim ordering
    # conv_base = VGG16(weights='imagenet',
    #                   include_top=False,
    #                   input_shape=(224, 224, 3))
    # model: object = models.Sequential()
    # model.add(conv_base)
    # assert model.get_input_shape_at(0) == (None, 224, 224, 32)
    # assert model.get_input_shape_at(1) == (None, 222, 222, 32)
    # assert model.get_input_shape_at(2) == (None, 111, 111, 64)
    # assert model.get_input_shape_at(3) == (None, 109, 109, 64)
    # model.add(layers.Flatten())
    # model.add(layers.Dense(512, activation='relu'))
    # model.add(Dense(num_classes, activation ='softmax'))
    # model.summary()

    # head_model = Model(input=conv_base.input, output=predictions)
    ...
    # target_layer = lambda x: target_c
    # head_model = Model(input = conv_base.input, output = predictions)
    ...
    if config.mode == 'train':
        ...
        opt = keras.optimizers.rmsprop(lr=0.00045, decay=1e-6) #0.001은 loss 증가, 기존 0.00045
